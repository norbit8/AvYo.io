...
setTable();
if (frameCount % 120 ==0){  // check for invites every 120 frames
socket.on('gotInvite', getTheInvite);
}
...
text("Nick: ", width/1.2, 115);
// TIMER (START)
text("Timer: ", width/1.2, 145);
//console.log(floor(timer/60) + ":" + parseInt(timer % 60, 10));
...
if(spammer != true) {
  socket.emit('invite', {whoAmI: socket.id, whoIWant: xxxx});
}
spam.push(xxxx);
